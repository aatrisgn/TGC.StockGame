...
var RegionDiscovery = class _RegionDiscovery {
  ...
  getRegionFromIMDS(version4, options) {
    return __async(this, null, function* () {
      return this.networkInterface.sendGetRequestAsync(`${IMDS_ENDPOINT}?api-version=${version4}`, options, IMDS_TIMEOUT);
    });
  }
  /**
...
var DEFAULT_TOKEN_BINDING_KEY_MANAGER = {
  provisionTokenBindingKey(request) {
    return __async(this, null, function* () {
      throw createClientAuthError(methodNotImplemented, request.correlationId);
    });
  },
  removeTokenBindingKey(_kid, correlationId) {
    return __async(this, null, function* () {
      throw createClientAuthError(methodNotImplemented, correlationId);
    });
  },
  getTokenBindingPublicKeyJwk(_kid, correlationId) {
    return __async(this, null, function* () {
      throw createClientAuthError(methodNotImplemented, correlationId);
    });
  }
...
var CacheManager = class {
  ...
  saveAccessToken(credential, correlationId, kmsi) {
    return __async(this, null, function* () {
      ...
      if (credential.additionalCacheKeyComponents && Object.keys(credential.additionalCacheKeyComponents).length > 0) {
        additionalCacheKeyHash = yield this.cryptoImpl.hashString(JSON.stringify(credential.additionalCacheKeyComponents));
      }
      ...
      yield this.setAccessTokenCredential(credential, correlationId, kmsi, additionalCacheKeyHash);
    });
  }
  /**
  ...
  credentialMatchesFilter(entity, filter2, correlationId) {
    ...
    if (!!filter2.target && !this.matchTarget(entity, filter2.target, correlationId)) {
      return false;
    }
    if (entity.credentialType === CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME) {
      if (!this.matchAccessTokenWithAuthScheme(entity, filter2)) {
        return false;
      }
    }
    const entityComponents = entity.additionalCacheKeyComponents;
    const filterComponents = filter2.additionalCacheKeyComponents;
  ...
  matchAccessTokenWithAuthScheme(entity, filter2) {
    const normalizedFilterTokenType = filter2.tokenType?.toLowerCase();
    if (!!filter2.tokenType && !this.matchTokenType(entity, filter2.tokenType)) {
      return false;
    }
    switch (normalizedFilterTokenType) {
      case "dpop":
        return this.matchKeyBoundAccessToken(entity, filter2, true);
      case AuthenticationScheme.SSH:
        return this.matchKeyBoundAccessToken(entity, filter2, false);
      default:
        return true;
    }
  }
  matchKeyBoundAccessToken(entity, filter2, requireKeyId) {
    if (!filter2.keyId) {
      return !requireKeyId;
    }
    return this.matchKeyId(entity, filter2.keyId);
  }
  /**
...
var PopTokenGenerator = class {
  ...
  generateKid(request) {
    return __async(this, null, function* () {
      const kidThumbprint = yield this.tokenBindingKeyManager.provisionTokenBindingKey({
  ...
  signPayload(payload, keyId, request, claims) {
    return __async(this, null, function* () {
      ...
      const publicKeyJwk = yield this.tokenBindingKeyManager.getTokenBindingPublicKeyJwk(keyId, request.correlationId);
...
var ResponseHandler = class _ResponseHandler {
      ...
      try {
        ...
        if (handlingRefreshTokenResponse && !forceCacheRefreshTokenResponse && cacheRecord.account) {
          ...
          if (cachedAccounts.length < 1) {
            ...
            return yield _ResponseHandler.generateAuthenticationResult(this.cryptoObj, authority, cacheRecord, false, request, this.performanceClient, {
              idTokenClaims,
   ...
   * @param stateString
   */
  static generateAuthenticationResult(_0, _1, _2, _3, _4, _5) {
    return __async(this, arguments, function* (cryptoObj, authority, cacheRecord, fromTokenCache, request, performanceClient, options = {}) {
...
function sendPostRequest(thumbprint, tokenEndpoint, options, correlationId, cacheManager, networkClient, logger, performanceClient) {
  return __async(this, null, function* () {
    ...
    let response;
    try {
      response = yield invokeAsync(networkClient.sendPostRequestAsync.bind(networkClient), NetworkClientSendPostRequestAsync, logger, performanceClient, correlationId)(tokenEndpoint, __spreadProps(__spreadValues({}, options), { correlationId, performanceClient }));
...
function computeJwkThumbprint(publicJwk, correlationId) {
  return __async(this, null, function* () {
...
var AsyncMemoryStorage = class {
  ...
  getKeys(correlationId) {
    return __async(this, null, function* () {
      ...
      try {
        const persistentCacheKeys = yield this.indexedDBCache.getKeys();
...
var TokenBindingKeyManager = class _TokenBindingKeyManager {
  ...
  provisionTokenBindingKey(request) {
    return __async(this, null, function* () {
      ...
      try {
        const activeKeyPair = yield this.createTokenBindingKey(request);
   ...
   * @param correlationId - Request correlation identifier.
   */
  getTokenBindingPublicKeyJwk(keyId, correlationId) {
    return __async(this, null, function* () {
      const cachedKeyPair = yield this.getTokenBindingKeyPair(keyId, correlationId);
      return exportJwk(cachedKeyPair.publicKey);
    });
  }
  createTokenBindingKey(request) {
    return __async(this, null, function* () {
      const keyGenAlgorithm = this.getTokenBindingKeyGenAlgorithmOptions(request.tokenBindingKeyAlgorithm, request.correlationId);
      const generatedKeyPair = yield this.generateKeyPairAndThumbprint(_TokenBindingKeyManager.TOKEN_BINDING_KEY_USAGES, keyGenAlgorithm, request.correlationId);
      yield this.cache.setItem(generatedKeyPair.keyId, __spreadProps(__spreadValues({}, generatedKeyPair), {
        tokenBindingKeyType: request.tokenBindingKeyType,
        tokenBindingKeyAlgorithm: request.tokenBindingKeyAlgorithm
      }), request.correlationId);
  ...
  generateKeyPairAndThumbprint(usages, keyGenAlgorithm, correlationId) {
    return __async(this, null, function* () {
      const keyPair = yield generateKeyPair(false, usages, keyGenAlgorithm);
      const publicJwk = yield exportJwk(keyPair.publicKey);
      const keyId = yield computeJwkThumbprint(publicJwk, correlationId);
      return {
  ...
  /** @internal */
  getTokenBindingKeyPair(keyId, correlationId) {
    return __async(this, null, function* () {
      const cachedKeyPair = yield this.cache.getItem(keyId, correlationId);
      if (!cachedKeyPair) {
...
var CryptoOps = class {
  ...
  removeTokenBindingKey(kid, correlationId) {
    return __async(this, null, function* () {
      yield this.tokenBindingKeyManager.removeTokenBindingKey(kid, correlationId);
    });
  }
  /**
  ...
  signTokenBindingJwt(header, payload, kid, correlationId) {
    return __async(this, null, function* () {
      ...
      try {
        const cachedKeyPair = yield this.tokenBindingKeyManager.getTokenBindingKeyPair(kid, correlationId);
        yield this.validateTokenBindingJwtHeaderKey(header, kid, correlationId);
        ...
        const encodedSignature = yield this.signInput(cachedKeyPair, tokenString, signingAlgorithm);
  ...
  signInput(cachedKeyPair, signingInput, algorithm) {
    return __async(this, null, function* () {
      const encoder = new TextEncoder();
      const signatureBuffer = yield sign(cachedKeyPair.privateKey, encoder.encode(signingInput), algorithm);
  ...
  validateTokenBindingJwtHeaderKey(header, kid, correlationId) {
    return __async(this, null, function* () {
      if (!header.jwk) {
        return;
      }
      const headerKeyId = yield computeJwkThumbprint(header.jwk, correlationId);
      if (headerKeyId !== kid) {
...
var BrowserCacheManager = class _BrowserCacheManager extends CacheManager {
   ...
   */
  setAccessTokenCredential(accessToken, correlationId, kmsi, additionalCacheKeyHash) {
    return __async(this, null, function* () {
...
var PlatformAuthInteractionClient = class extends BaseInteractionClient {
  ...
  acquireToken(request, cacheLookupPolicy) {
    return __async(this, null, function* () {
      ...
      let nativeRequest;
      try {
        nativeRequest = yield this.initializePlatformRequest(request);
      } catch (e) {
      ...
      try {
        ...
        const result = yield this.handleNativeResponse(validatedResponse, nativeRequest, reqTimestamp, request.storeInCache);
        nativeATMeasurement.end({
          success: true,
  ...
  handleRedirectPromise() {
    return __async(this, null, function* () {
      ...
      try {
        ...
        const authResult = yield this.handleNativeResponse(response, request, reqTimestamp, storeInCache);
        const serverTelemetryManager = initializeServerTelemetryManager(this.apiId, this.config.auth.clientId, this.correlationId, this.browserStorage, this.logger, void 0, this.config.system.serverTelemetryEnabled);
        serverTelemetryManager.clearNativeBrokerErrorCode();
        return authResult;
      } catch (e) {
   ...
   * @param reqTimestamp
   */
  handleNativeResponse(response, request, reqTimestamp, storeInCache) {
    return __async(this, null, function* () {
   ...
   * @param reqTimestamp
   */
  cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, tenantId, reqTimestamp, environment, storeInCache) {
    return __async(this, null, function* () {
...
function createForm(frame, action, fields) {
  ...
  form.method = "post";
  form.action = action;
  Object.keys(fields).forEach((name4) => {
    const input = frame.createElement("input");
    input.hidden = true;
    input.name = name4;
    input.value = fields[name4];
...
var PlatformAuthDOMHandler = class _PlatformAuthDOMHandler {
  ...
  initializePlatformDOMRequest(request) {
    ...
    const _a = request, { accountId, clientId, authority, scope, redirectUri, correlationId, state, extraParameters } = _a, remainingProperties = __objRest(_a, ["accountId", "clientId", "authority", "scope", "redirectUri", "correlationId", "state", "extraParameters"]);
  ...
  hydrateCache(result, request) {
    return __async(this, null, function* () {
      ...
      if (result.fromPlatformBroker) {
        ...
        const additionalCacheKeyHash = components ? yield this.browserCrypto.hashString(JSON.stringify(components)) : void 0;
        ...
        yield this.nativeInternalStorage.setAccessTokenCredential(accessTokenEntity, result.correlationId, kmsi, additionalCacheKeyHash);
      } else {
...
_MsalBroadcastService.\u0275fac = function MsalBroadcastService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _MsalBroadcastService)(\u0275\u0275inject(MSAL_INSTANCE), \u0275\u0275inject(NgZone), \u0275\u0275inject(MSAL_BROADCAST_CONFIG, 8));
};
...
@azure/msal-common/dist-browser/crypto/ITokenBindingKeyManager.mjs:
...
@azure/msal-common/dist-browser/utils/ObjectUtils.mjs:
...
@azure/msal-browser/dist/crypto/TokenBindingKeyManager.mjs:
...
*/
//# sourceMappingURL=chunk-3SQQMRC2.js.map
